    ...
    if (!deleted) {
        ...
        currentEditorValue.child("content").once("value", function (contentRef) {
            ...
            if (val === null) {
                ...
                // ...with this content:
                val = "/* Welcome to 404Editor v1.0.2! */";
...
function delWork(){
    if ($("#workspaceName").text() == "Workspace: PUBLIC") {
        alert("You can't delete this workspace!");        
    }
    else{
        if (confirm("Are you sure you want to delete this workspace?\n\nThis cannot be undone.")){
            location.href = "../editor";
            // setTimeout(function(){editorValues.child(editorId).remove();}, 500);
            editorValues.child(editorId).remove();
            deleted = true;
        }
    }
}
...
function download(){
    alert("Downloading your file!\nIn a moment, right-click and choose 'Save as...' to save your file!");
    var text = []; 
    var x = 0;
    ...
    var name = $("#workspaceName").text().substring(11);
    var lang = $("#select-lang option:selected").text();
    var textString = "";
    // console.log(textString);

    // let fso = CreateObject("Scripting.FileSystemObject");
    // let s = fso.CreateTextFile("E:/test.txt", true);
    ...
    $.ajax({
        url: 'save.php',
        type: 'POST',
        data: ({value: textString, lang: lang, name: name}),
...
function openList() {
    document.getElementById("workspaceList").style.width = "250px";
}

function closeList() {
    document.getElementById("workspaceList").style.width = "0";
}

function showCreate() {

    if(document.getElementById("createbox").style.height != "0px"){
        document.getElementById("createbox").style.height = "0px";
        document.getElementById("dropper").innerHTML = "Create/Join Workspace <i class='fa fa-chevron-down'></i>"
    }
    else{
        document.getElementById("createbox").style.height = "60px";
        document.getElementById("dropper").innerHTML = "Create/Join Workspace <i class='fa fa-chevron-up'></i>"
    }
}
