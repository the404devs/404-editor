...
$(function() {
    if (!deleted) {
        ...
        var editorPass = Url.queryString("pass") || null;
        ...
        currentEditorValue.child("content").once("value", function (contentRef) {
            ...
            var passLocation = db.ref("editor_values").child(editorId).child("password");
            // alert(passLocation);
            passLocation.on("value", function (getPass) {
                // alert(getPass.val());
                if(editorPass == getPass.val()){
                    // alert("correct pass");
                }
                else{              
                    if (editorId != "PUBLIC")
                    {
                        ...
                        location.href = "http://the404.ml/editor"; 
                    }                            
                }
            });
...
function download(){
    ...
    function determineLang(){
        ...
        lang = getSelectedOption(document.getElementById("select-lang")).id;
        // alert(lang);
    }

    determineLang();
    var fullname = editorId + lang;
    // alert(fullname);
    save(textString, fullname, "txt"); 
}

function showCreate() {
    if(document.getElementById("createbox").style.height != "130px"){
        document.getElementById("createbox").style.height = "130px";
